...
//need to delete
const STORAGE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweGJDMjA0MGM3NEM3MGNlRTYyMTMyNDk4Qjg1ZkEwYzQyMDM3MjM4RTciLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY2NzI5OTQ2MjIxOCwibmFtZSI6IlBTUDM0In0.8tpKbH8eNXdUUST5h0OTOvn-EKeiPhLeZSCLPR6uOk4";

const Main = () => {
  const day = new Date();
  const today = day.toISOString().replace("T", "-").slice(0, -8);
 ...
 */
  ...
  const setupContract = async () => {
    try {
      ...
      const mintFunction = await psp34.tx.mintWithAttribute(
        { gasLimit },
        name,
        currentAccount,
        date,
  ...
  const CreateCollection = async (collectionName, symbol) => {
    try {
      ...
      const Id = 1; //todo
      const createcollection = code.tx.new(
        { gasLimit, storageDepositLimit },
        Id,
        collectionName,
        symbol
      );
      createcollection.signAndSend(
        currentAccount.address,
        { signer: injector.signer },
        ({ contract, status }) => {
          if (status.isInBlock) {
        ...
        <div className={style.formHeader}>
          <div
            className={`${style.activetab} ${
              activeTab !== "collection" && style.inactivetab
            }`}
          ...
          <div
            className={`${style.activetab} ${
              activeTab !== "mint" && style.inactivetab
            }`}
